% write a routine to load the data.
% reading in the data from the instron.
% show the end point on the graph.
...
'x_ord_choices',{x_ord_choices} ,'y_ord_choices' ,{y_ord_choices},'x_ord_choice',x_ord_choice,'y_ord_choice',y_ord_choice));  
...
handles.slider_handle_2   = uicontrol('Style','slider','Min',1,'Max',length(movie_data),'Value',frame_to_view,'units','normalized','Position' ,  [0.02 0.79 0.1 0.03],  'Callback' , @slider_func_2)  ; 
...
% at this point mark in the projected end
...
frame_rate                     =   plot_data_structure.movie_data.FrameRate                                      ;
correct_number_of_objects      =   length(plot_data_structure.detected_object_properties.Detected_Object_List)   ;
number_of_frames               =   length(plot_data_structure.movie_data)                                        ;   
...
detected_object_properties_temp = find_detected_objects (plot_data_structure.movie_data,index,threshold_val,min_detection_size,max_circ_tol);
...
save whole_test_values  whole_test_values
...
hold off
imshow(plot_data_structure.movie_data(plot_data_structure.frame_to_view).gray)
else    
plot_data_structure.pixels_per_mm = NaN;            
end
...
BW1 = im2bw(plot_data_structure.movie_data(plot_data_structure.frame_to_view).gray,plot_data_structure.threshold_val);

BW1 = im2bw(plot_data_structure.movie_data(plot_data_structure.frame_to_view).gray,plot_data_structure.threshold_val);
axes(plot_data_structure.handles.axis_2)
cla(plot_data_structure.handles.axis_2)
hold on
imshow(BW1)
...
movie_data(1:N_frames) = struct('gray',zeros(height_,width_,'uint8'),'colormap',[],'FrameRate',FrameRate);


for index = 1 : N_frames
  
%movie_data(index).gray = rgb2gray(read(video_obj,index));
movie_data(index).gray = rot90(rgb2gray(read(video_obj,index)));
